...
# ── NEW: Scenario / Descriptive schemas ──────
...
class ScenarioQuestion(BaseModel):
    ...
    question: str
    context: str       # short background / scenario setup (may be empty for descriptive)
    sampleAnswer: str  # model answer / key points expected
    evaluationCriteria: List[str]  # bullet-point rubric items
...
class EvaluateFeedback(BaseModel):
    score: int            # 0-10
    grade: str            # e.g. "Excellent", "Good", "Needs Improvement"
    strengths: List[str]
